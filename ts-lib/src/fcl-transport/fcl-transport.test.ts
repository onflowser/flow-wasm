...
import { Account, Block, Transaction } from "@onflow/typedefs";
...
describe("FCL transport - accounts", () => {
  ...
  it("should get account by address", async () => {
    const actual = await fcl
      .send([fcl.getAccount("0xf8d6e0586b0a20c7")])
      .then(fcl.decode);
      ...
      keys: [
        {
          ...
          index: 0,
          publicKey:
            "0x43661ddd40c0510b2097a5ad583607f4780876184308a325516951fac6a816fe4e522c9278d3ef3d67c6d903291d0501f9a9bd5b4dc2c5af26c2ad0597bac97a",
  ...
  it("should get block by id", async () => {
    const actual = await fcl
      .send([
        fcl.getBlock(),
        fcl.atBlockId(
          "a20c602fbee6fe4491e116403e3258e7b7924609696ab2edb9a93eed2c29e445"
        ),
      ])
      .then(fcl.decode);
...
describe("FCL transport - collections", async () => {
  ...
  it("should return collection by ID", async () => {
    ...
    const block = await fcl
      .send([fcl.getBlock(), fcl.atBlockId(status.blockId)])
      .then(fcl.decode);
    ...
    const actual = await fcl
      .send([fcl.getCollection(collectionId)])
      .then(fcl.decode);
...
describe("FCL transport - transactions", async () => {
  ...
  it("should execute a simple transaction with arguments", async () => {
    const transactionId = await fcl.mutate({
      ...
      args: (arg: any, t: any) => [arg("Hello from outside", t.String)],
      limit: 10,
    });
  ...
  it("should get transaction by ID", async () => {
    ...
    const expected: Omit<
      Transaction,
      "proposer" | "address" | "keyId" | "sequenceNumber"
    > = {
